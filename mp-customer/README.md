...
```
首页    门店介绍（地址/电话/营业时间）+ 近期活动（能领的券）
点餐    这家店的菜单 → 选规格 → 购物车 → 下单 → 支付
订单    下单记录和状态
我的    储值余额 / 积分 / 券包；没登录时给的是登录入口
```

**门店存在本地**（`src/stores/currentStore.ts`）。改成常驻 tab 之后，
没有「选门店 → 跳过去」那一步了，点餐页得知道自己点的是哪家店——
首页那张卡片负责显示和切换。换门店会清空购物车（A 店点的菜到了 B 店可能没有）。

**点餐和查订单全程不需要登录**；会员（余额、积分、券）是可选的，
用手机号 + 验证码登一下就有。「我的」那一页未登录时不是一道墙，
而是一个入口 + 一句「不登录也能点单」。

没登录时订单的 `member_id` 是空的，「我的订单」靠本地存的单号 + 查询令牌；
登录之后下单会挂上会员（积分才有地方攒）。
