...
我的班表   接下来哪天上班、几点到几点（schedule:view）
经营       店长看本店、老板看全公司（report:store / report:all）
```

**菜单、员工、排班那些不在这儿**。手机屏幕塞不下，硬塞进去只会让每件事都变难用——
那些是电脑上的活。首页上也是这么写的。

排班是个例外，但要分清两个看法：**店长在电脑上「排班」**（一周的格子，
点格子勾班次），**员工在手机上看「我的班表」**（只看自己哪天来）。
同一套数据两个入口，因为这两个人在这些事上要的东西完全不同。

每个入口和按钮都按登录的人有没有那个权限码来渲染，登录不同账号能看到的东西不一样：

| 账号 | 登录后能看到 |
| --- | --- |
| `shouyin` 收银员 | 点单、接单出单、核销、我的班表 |
| `fuwuyuan` 服务员 | 「代客点单」+「我的班表」（他没有 `order:receive`） |
| `houcu` 后厨 | 「看单」+「我的班表」——**他连接单都点不了**，见下面那节 |
| `dianzhang` 店长 | 再加上「经营」 |

**「我的班表」是唯一一个所有角色都有的入口**——排班是店长的活，
但看自己哪天上班是每个人的事。
