...
| `yunying` | 运营主管 | 菜单管理（菜品/分类/门店菜单）、门店查看；**看不到**员工和审计 |
| `caiwu` | 财务 | 订单、门店；**碰不了菜单** |
| `dianzhang` | 店长 | 本店订单、本店菜单定价、本店员工；**看不到别家店**，也改不了菜品基础信息 |
| `zhiban` | 值班经理 | 收银员的全部 + 取消订单、审批退款；**批不了大额退款** |
| `shouyin` | 收银员 | 订单（接单/收款）；**没有取消订单的权限** |
| `fuwuyuan` | 服务员 | 订单；只能代客点单，不能收款 |
| `gongyong` | 服务员（公用账号） | 同上——注意它的订单记的是「实际操作人」而不是账号本身 |
| `houcu` | 后厨 | 只有订单，用来看单出餐 |
...
- **订单交易** —— 下单（后端算价）、接单、完成、取消、组合支付（一单多笔收款）
