...
| | 地址 | 建议 |
| --- | --- | --- |
| 内部人员网页端 | <http://120.26.65.242/> | 电脑打开。`dianzhang` / `Demo123!`（店长）、`laoban` / `Demo123!`（老板） |
| 顾客端 | <http://120.26.65.242/customer/> | **手机打开**——扫码点单那一套（首页 → 点餐 → 下单 → 我的） |
| 员工端 | <http://120.26.65.242/staff/> | **手机打开**——代客点单、出单、我的班表 |
| 接口文档 | <http://120.26.65.242/api/docs> | Swagger，从后端路由自动生成 |

> **为什么是 IP 不是域名**：大陆服务器上，**没备案的域名不管哪个端口都会被云厂商拦掉**
> （实测过，换端口也不行）。所以演示走 IP——备案是另一条路，1~3 周。
> 完整记录见 [docs/部署.md](docs/部署.md)。
>
> 演示数据是**一次性**的：谁都能用演示账号登进去点单、改店名。别把它当成真环境。
...
cd backend
flask db upgrade
python manage.py seed-rbac             # 39 个权限码 + 8 个预置角色
python manage.py seed-demo             # 6 家门店、15 道菜、11 个账号、12 笔订单、券和会员、两周排班
...
**内存小的机器（1~2G）另有一条路**：三个前端改在**本地构建**、打包传上去，
服务器只装 Python——不然 BuildKit 会并行跑三个 node 构建，把小机器打满
（实机上 ssh 都登不进去，得去控制台强制重启）。命令见部署文档。

完整步骤、开机自检流程（为什么 `seed-rbac` 不能省）、小内存机器那条路、nginx 配置，
以及**一张按「现象」查的排查表**（十五条，全是真机踩出来的：从
`download.docker.com` 被墙到「整个机器不响应、控制台都进不去」），
...
| 账号 | 角色 | 登录后能看到 |
| --- | --- | --- |
| `laoban` | 老板 | **全公司管理层**：门店、账号、菜单定价、报表财务、审批退款、对账、审计。**看不到「点单」**——他不代客点单、不收银、不核销，那些是柜台的活 |
...
而**「我的班表」人人都有**（排班是店长排的，看自己哪天上班是每个人的事）。
...
- **权限体系** —— 39 个权限码 × 8 个预置角色 × 数据范围（本店/全部），后端强制、前端按权限渲染
...
- **员工小程序**（二期）—— 代客点单、接单出单、团购券核销、我的班表。只放门店一线
  用得上的几件事，**菜单/员工/报表那些不在这儿**（手机屏幕塞不下，硬塞只会让每件事都变难用）
  ...
  **「我的班表」是唯一一个所有角色都有的入口**（`schedule:view`）

  **店长多一个「经营」**（`report:store`）——和电脑端**同一份数据**：
  汇总、趋势、支付构成、菜品排行、时段分布、门店对比。
  一线的人不需要「今天做了多少生意」，这个数也不该给他们

  老板在小程序上只有「看单」和「经营」两件事：他不代客点单、不核销，
  站在店里也是看，不是干（`docs/设计决策.md` 第 19 条讲了这个边界怎么划的）
...
- **经营报表**（三期）—— **一个接口两个角色**：店长看本店、老板看全公司，
  ...
  **两个端的交互不一样，因为输入设备不一样**：电脑上是鼠标悬停出 tooltip；
  手机上没有悬停，改成点柱子、在标题右边显示那一根的日期和金额。
  刻度也是——两周以内写日期，再长就写**序号**（第几天）：
  30 天每根柱子才 22px，日期标签放不下，硬塞就得抽稀，一抽稀就参差不齐

<img src="docs/screenshots/reports.png" width="820" alt="经营报表" />

- **排班**（三期）—— 店长在电脑上排一周的班（**按人分行、按天分列**的格子，
  点格子勾班次，一天可以勾两个），员工在手机上看「我哪天上班」。
  **这不是考勤**：排班回答的是「下周三谁上早班」，不是「昨天张三几点几分打的卡」——
  打卡要设备、要跟工资挂钩、要处理补卡，是另一个系统的事

  班次和排班**分两张表**：班次是门店级配置（早班 09:30-14:00），排班是「哪天谁上哪个」。
  合成一张的话每次排班都要重填一遍起止时间，而且改一次班次时间就得改一批排班记录，
  改漏一条就出现两个时间不一样的「早班」

  **班次是门店级的**：西溪印象城店 10:30 才开门，它的「早班」和别家不是一个时间。
  **排过班的班次删不掉、只能停用**——那个名字底下挂着几十条记录，删了它们就没头没尾了

  权限分两档：`schedule:view`（看）和 `schedule:manage`（排）。服务员也有「看」——
  他得知道自己哪天来；但改排班是店长的活

<img src="docs/screenshots/schedule.png" width="820" alt="排班" />
<img src="docs/screenshots/mp-staff-mine.png" width="300" alt="员工端：我的班表" />

- **审计日志** —— 改价、上下架、发券、开停账号等关键动作自动留痕，含变更前后值
- **角色权限矩阵** —— 8 个角色 × 39 个权限码的全貌，一眼看出谁能在哪些门店做什么

## 几个设计上的取舍

这部分是这个项目里最值得看的东西——每个决定都有具体的理由，不是随手写的。
完整版（19 条）在 **[docs/设计决策.md](docs/设计决策.md)**，下面挑四条讲。
